...
class ImarisWriterTest {
	...
	void doConvert16(int aTestIndex) {
		...
		// Specify image size in pixels
		BPConverterTypesC_Size5D aImageSize = new BPConverterTypesC_Size5D();
		aImageSize.mValueX = 2048;
		aImageSize.mValueY = 2048;
		aImageSize.mValueZ = 100;
		aImageSize.mValueT = 10;
		aImageSize.mValueC = 3;
      ...
      for (bpSize vVoxelIndex = 0; vVoxelIndex < 2 * vInputBlockSize; ++vVoxelIndex) {
		 ...
		 */
		MersenneTwister mt = new MersenneTwister(33);
		double vPrevious = mt.nextGaussian()*40 + 100;
